...
- を格で自動詞（消え去る/輝く）→ 歪み。逃げていくは経路のを格として許容
- に格/から格で他動詞（砕く/守る）→ 歪み。自動詞はOK
...
#### 存在シート + 状況テンプレート方式
1. **存在シート**（mod×subj = 1枚）: interpretation（デフォルトの絵）+ alt（文脈別の読み替え）
   - 例: 壊れた子猫 → interpretation「ぬいぐるみ。綿が出ている」alt「機械仕掛けの猫（人工物文脈）/ 病気の猫（生死文脈）」
   - interpretationは確定描写ではなく、テンプレートが拾える程度の情報量
   - altは2つ程度。括弧内にどのノード文脈で使うかのヒント付き
   - **創造的解釈が先、特性体系は後**（解釈を書いてからテンプレート分岐に必要な軸を帰納的に見つける）
   - 必要数: mod数×subj数（現候補: 11×17=187枚）。ここが創造性の集中投資先
   - フォーマット検証済み: 子猫（具象）×11修飾語、沈黙（抽象）×11修飾語で成立確認
